...
wss.on('connection', function(ws) {
    ...
    ws.on('message', function(_message) {
        ...
        case 'p2p_call':
            p2p_call(sessionId, message.to, message.from, message.sdpOffer);
            break;

        case 'p2p_call_incoming_response':
            p2p_call_incoming_response(sessionId, message.from, message.sdpOffer);
            break;
...
function broadcast(msg) {
    var my_peer = userRegistry.getByName(msg.my_name);
    var other_peer = userRegistry.getByName(msg.other_name);

    my_peer.sendMessage(msg);
    other_peer.sendMessage(msg);
};


function p2p_call_incoming_response(id, from, sdpAnswer){
    
    // recuperando receptor.
    var receptor = userRegistry.getById(id);
...
function call(callerId, to, from, sdpOffer) {
    ...
    if (userRegistry.getByName(to)) {
        var callee = userRegistry.getByName(to); // nome de quem recebe ligação
        caller.sdpOffer = sdpOffer  //Oferta de quem liga
        callee.peer = from;  // nome de quem liga
        caller.peer = to;  //  
        var message = {
            id: 'incomingCall',
            from: from
...
// Função p2p
function p2p_call(callerId, to, from, sdpOffer){

    var chamador = userRegistry.getById(callerId);

    if(userRegistry.getByName(to)){
        var receptor = userRegistry.getByName(to); // nome do receptor
        chamador.sdpOffer = sdpOffer;  //Oferta do chamador
        receptor.peer = from;  // nome do chamador
        chamador.peer = to;  //  nome do receptor

        // gerando mensagem
        var message = {
            id: 'p2p_call_incoming',
            from: from,
            sdpOffer : sdpOffer
        };

        try{
            return receptor.sendMessage(message);
        } catch(exception) {
            rejectCause = "Error " + exception;
        }
    }
...
function test(name, ws){
    console.log('##################');
    console.log('name: ' + name);
    console.log('ws: ' + ws);
    ws.send(JSON.stringify({id: 'test_resp', response: 'servidor responde'}));
}
